...
var (
	...
	// SaveBlockChain - An alarm that tells the client to save the chain once a day
	SaveBlockChain = time.NewTimer(24 * time.Hour)
)

const (
	// SaveBlockChainAfter - Wait two seconds after resetting the timer
...
func HandleNetBlock(newbl *network.BlockRcvd) {
	if newbl.Block == nil {
		L.Debug("Network sent empty block")
	} else {
		defer func() {
		...
		if e := LocalAcceptBlock(newbl); e != nil {
			...
			L.Debug("block ", newbl.Block.Height, " accepted")
	...
	if common.FLAG.Rescan && common.FLAG.VolatileUTXO {
		...
		for !usif.ExitNow.Get() {
			...
			case <-common.KillChan:
				L.Debug("SIGNAL: Exit Now Priority")
				common.Busy()
				usif.ExitNow.Set()
				continue
			case newbl := <-network.NetBlocks:
				L.Debug("SIGNAL: Net Blocks Priority")
				common.Busy()
				HandleNetBlock(newbl)
			case rpcbl := <-rpcapi.RPCBlocks:
				L.Debug("SIGNAL: RPC Blocks Priority")
				common.Busy()
			...
			select {
			case <-common.KillChan:
				L.Debug("SIGNAL: Exit Now")
				common.Busy()
				usif.ExitNow.Set()
				continue
			case newbl := <-network.NetBlocks:
				L.Debug("SIGNAL: Net Blocks")
				common.Busy()
				HandleNetBlock(newbl)
			case rpcbl := <-rpcapi.RPCBlocks:
				L.Debug("SIGNAL: RPC Blocks")
				common.Busy()
				HandleRPCblock(rpcbl)
			case rec := <-usif.LocksChan:
				L.Debug("SIGNAL: rec")
			...
			case <-SaveBlockChain.C:
				L.Debug("SIGNAL: Save Blockchain")
				common.Busy()
			...
			case newtx := <-network.NetTxs:
				L.Debug("SIGNAL: New Transaction")
				common.Busy()
		...
		L.Debug("Finished processing signals")
